...
def diag_init():
    return {
        ...
        "t_meta": 0.0, "t_rank": 0.0, "t_mis": 0.0, "t_yf": 0.0, "t_filter": 0.0, "total": 0.0
    }
...
def fast_mis_scan(meta_dict, status_placeholder, now_ts, is_test, diag):
    ...
    for i in range(0, len(codes), CHUNK_SIZE):
        if mis_diag["mis_req_err"] >= 3:
            diag_err(diag, Exception("MIS_CIRCUIT_BREAK"), "MIS_BREAK")
            break
...
if st.button("🚀 啟動熱門資金狙擊"):
    if now_time - last_run < cooldown_seconds:
        st.warning(f"⏳ 系統冷卻中，請等待 {int(cooldown_seconds - (now_time - last_run))} 秒...")
    else:
...
if scan:
    ...
    with st.expander("⚙️ 系統診斷與底層監控 (白盒分析)", expanded=False):
        ...
        st.caption(f"🛡️ MIS 探針：HTTP 錯誤 {d.get('mis_http_err',0)} | 格式異常 {d.get('mis_ct_err',0)}")
        ...
        st.caption(f"耗時分布：Meta {d['t_meta']:.2f}s | 排行榜 {d['t_rank']:.2f}s | MIS {d['t_mis']:.2f}s | YF {d.get('t_yf',0):.2f}s | Filter {d['t_filter']:.2f}s")
