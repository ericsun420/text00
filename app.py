...
def split_underscore_nums(s: str):
    if not s: return []
...
# =========================
# Universe — GitHub ONLY (fixed URLs)
# =========================
@dataclass
class Meta:
    code: str
...
def render_table_html(title: str, df: pd.DataFrame, columns: List[str], height: int = 560) -> None:
    ...
    <style>
      ...
      thead th {{ position: sticky; top:0; z-index:2; text-align:left; padding: 11px 10px;
        background: rgba(15,17,22,.98); border-bottom: 1px solid var(--line);
        white-space: nowrap; font-weight: 900; }}
      tbody td {{ padding: 10px 10px; border-bottom: 1px solid rgba(148,163,184,.10);
...
<div class="header-wrap">
  <div>
    ...
    <div class="subtitle">先出畫面 → 你按掃描才抓 GitHub 清單（避免白畫面）</div>
  </div>
...
if run_scan:
    ...
    with st.spinner("Step 1/3：下載股票清單（GitHub raw）..."):
        try:
    ...
    st.success(f"清單 OK：{len(meta_items):,} 檔")

    with st.spinner("Step 2/3：MIS 抓盤中即時快照..."):
    ...
    with st.expander("📋 族群共振排行榜（Top 10）", expanded=True):
        render_table_html("族群共振排行榜", sector,
                          ["排名","族群名稱","共振分","熱檔數","貼板數","掃描檔數","平均熱度","最高熱度","平均量"], height=420)
    ...
    with st.spinner(f"Step 3/3：抓候選日線基準（{len(cand_syms)} 檔）..."):
        base = build_daily_baseline_for_candidates(cand_syms)

    if base is None or base.empty:
        st.error("候選日線抓不到（yfinance 可能被限流）。請稍後重試或把候選縮小。")
        st.stop()
    ...
    for _, r in pre.iterrows():
        ...
        if max_ret_10d >= hype_thr:  # 排除「不是第一根」
            continue
    ...
    res = pd.DataFrame(rows).sort_values(["連板潛力分","距離漲停(%)","盤中爆量倍數(線性)"],
    ...
    with st.expander("📋 完整榜單（美化表格）", expanded=True):
        cols_show = [
            ...
            "累積量(張)","盤中爆量倍數(線性)","基底天數","近5日漲幅(%)","ATR20(%)","基底緊縮分","連板潛力分"
        ]
