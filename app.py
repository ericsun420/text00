...
# ✅ 日線基準只對少量候選抓（避免卡在建立日線）
#
# 安裝：
#   pip install -U streamlit pandas yfinance requests urllib3
# （建議）pip install -U twstock   # 有裝更穩：清單不必走 GitHub fallback
#
# 執行：
#   streamlit run app.py

import io  # ✅ FIX: 之前漏這行，沒裝 twstock 時會直接炸掉
import math
import time
import html
import re
...
<style>
:root{
  --bg:#07080b; --panel:#0b0d12; --card:#0f1116;
...
def load_universe(include_tpex: bool) -> Dict[str, Meta]:
    ...
    try:
        ...
        for _, info in twstock.codes.items():
            ...
            if not re.match(r"^\d{4,6}$", c or ""):
                continue
            if m in ("上市", "TWSE"):
                meta[c] = Meta(c, n, "上市", g or "未分類", "tse", f"{c}.TW")
            elif m in ("上櫃", "TPEX"):
                if not include_tpex:
                    continue
                meta[c] = Meta(c, n, "上櫃", g or "未分類", "otc", f"{c}.TWO")
        if len(meta) > 300:
            return meta
    except Exception:
        pass
    ...
    for _, r in df1.iterrows():
        ...
        if re.match(r"^\d{4,6}$", c or ""):
            meta[c] = Meta(c, str(r.get("name","")).strip(), "上市", str(r.get("group","")).strip() or "未分類", "tse", f"{c}.TW")
    ...
    if include_tpex:
        ...
        for _, r in df2.iterrows():
            c = str(r.get("code","")).strip()
            if re.match(r"^\d{4,6}$", c or ""):
                meta[c] = Meta(c, str(r.get("name","")).strip(), "上櫃", str(r.get("group","")).strip() or "未分類", "otc", f"{c}.TWO")

    if len(meta) < 200:
        raise ValueError("清單來源不足。建議：pip install twstock")
    return meta
...
class MISClient:
    ...
    def get_quotes(self, ex_ch_list: List[str]) -> List[dict]:
        ...
        if not ex_ch_list:
            return []
        ex_ch = "%7c".join(ex_ch_list)  # '|' 的 url-encoding
...
# =========================
# Daily baseline for candidates only
# =========================
...
# =========================
PRESETS = {
    "保守": dict(near_ticks=1, min_lots=1500, dist_upper=0.8),
    "標準": dict(near_ticks=1, min_lots=1200, dist_upper=1.0),
    "積極": dict(near_ticks=2, min_lots=800,  dist_upper=1.5),
}
...
min_lots = st.sidebar.number_input("最低盤中量（張）", min_value=200, max_value=20000, value=int(PRESETS[mode]["min_lots"]), step=100)
dist_upper = st.sidebar.number_input("候選距離漲停(%)", min_value=0.1, max_value=5.0, value=float(PRESETS[mode]["dist_upper"]), step=0.1)
st.sidebar.markdown("---")
...
<div class="header-wrap">
  <div>
    ...
    <div class="subtitle">主軸：抓第一根漲停 + 族群共振（MIS快照 → 少量候選再抓日線）</div>
  </div>
...
st.markdown("""
<div class="banner">
<b>重點：</b>這版不會再整頁消失（已修 io NameError），且盤中不靠 yfinance 5m（避免限流）。
</div>
""", unsafe_allow_html=True)
...
if run_scan:
    ...
    # simple sector radar from MIS
    df = snap.copy()
    df["industry"] = df["industry"].fillna("未分類").replace("", "未分類")
    df["near_upper"] = df["last"] >= (df["upper"] - df["upper"].fillna(0).apply(lambda u: tw_tick(float(u)) if u else 0.05))
    df["heat"] = 0.0
    ...
    sector["共振分"] = (
        sector["熱檔數"]*20.0 + sector["貼板數"]*8.0 +
        sector["平均熱度"]*0.35 + sector["最高熱度"]*0.25
    ).clip(0, 100)
...
""", unsafe_allow_html=True)
    ...
    st.markdown("<div class='hr'></div>", unsafe_allow_html=True)
    st.subheader("🚀 第一根漲停候選（候選少量才抓日線）")

    if pre.empty:
        st.warning("目前沒有符合『量大 + 接近漲停』的候選（可放寬距離漲停%或降低最低量）。")
        st.stop()
    ...
    if base is None or base.empty:
        st.error("候選日線抓不到（yfinance 可能被限流）。可縮小候選檔數或稍後重試。")
        st.stop()
    ...
    for _, r in pre.iterrows():
        ...
        score += min(15.0, max(0.0, (base_len - 10) * 0.4))
        score += min(15.0, max(0.0, base_tight * 15.0))
