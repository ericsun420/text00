...
def load_universe(include_tpex: bool) -> Dict[str, Meta]:
    ...
    # 2) GitHub raw (備用方案，加入 verify=False)
    def fetch_csv(url: str) -> pd.DataFrame:
        r = requests.get(url, timeout=15, allow_redirects=True, headers={"User-Agent": "Mozilla/5.0"}, verify=False)
        r.raise_for_status()
        text = r.text.replace("\r\n", "\n").replace("\r", "\n")
        # 移除 on_bad_lines 確保舊版 Pandas 也能跑
        return pd.read_csv(io.StringIO(text), dtype=str, engine="python")
    ...
    if len(meta) < 200:
        raise ValueError("清單來源不足（twstock與GitHub皆失敗）。請確認網路連線，或於終端機執行 pip install -U twstock")
...
def fetch_mis_snapshot(meta_items: List[Meta], batch_size: int = 70) -> pd.DataFrame:
    ...
    for i in range(0, len(meta_items), batch_size):
        ...
        for q in arr:
            ...
            rows.append({
                ...
                "upper": u, "lower": w, "volume_shares": v,
                "bid_p0": bid_p0, "bid_v0": bid_v0, "ask_p0": ask_p0, "ask_v0": ask_v0,
    ...
    df.loc[df["prev_close"].isna() | (df["prev_close"] == 0), "chg_pct"] = float("nan")
    return df
        ...
        for sym in syms:
            try:
                ...
                rows.append({
                    ...
                    "ret_5d": ret_5d, "max_ret_10d": max_ret_10d, "yday_upper_wick_ratio": y_upper_wick_ratio,
                    "yday_vol_spike": yday_vol_spike, "base_len_days": base_len_days, "base_tight_score": base_tight_score,
                })
            except Exception:
                continue
        time.sleep(0.05)
...
# =========================
PRESETS = {
    "保守（只抓幾乎鎖死、連板體質強）": dict(near_ticks=1, min_cum_lots=1500, min_close_pos=0.92, max_pullback_pct=0.35, max_ret_5d=8.0, max_atr20=4.8, min_base_len=28, min_base_tight=0.55, min_vol_ratio=2.3, min_bid_lots=200, max_ask_lots=60),
    "標準（平衡：第一根漲停 + 連板機率）": dict(near_ticks=1, min_cum_lots=1200, min_close_pos=0.88, max_pullback_pct=0.55, max_ret_5d=12.0, max_atr20=6.5, min_base_len=18, min_base_tight=0.45, min_vol_ratio=1.8, min_bid_lots=120, max_ask_lots=120),
    "積極（多抓：允許盤中較不穩）": dict(near_ticks=2, min_cum_lots=800, min_close_pos=0.82, max_pullback_pct=0.90, max_ret_5d=18.0, max_atr20=8.5, min_base_len=10, min_base_tight=0.35, min_vol_ratio=1.4, min_bid_lots=60, max_ask_lots=250),
}
...
def build_sector_radar(snapshot: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    sector["共振分"] = (sector["熱檔數"]*20.0 + sector["貼板數"]*8.0 + sector["平均熱度"]*0.35 + sector["最高熱度"]*0.25 + (sector["平均量"]/2000.0).clip(upper=5)*3.0).clip(0, 100)
...
try:
    ...
    st.error(f"🛑 啟動失敗：無法載入股票清單。錯誤訊息：{e}")
    st.stop()
...
""", unsafe_allow_html=True)
        ...
        with st.expander("📋 族群共振排行榜（Top 10）", expanded=True):
            render_table_html("族群共振排行榜", sector_rank, ["排名","族群名稱","共振分","熱檔數","貼板數","掃描檔數","平均熱度","最高熱度","平均量"], height=420)
        ...
        for sec in sector_rank["族群名稱"].tolist():
            ...
            with st.expander(f"📌 {sec}（Top {len(sub)}）", expanded=False):
                render_table_html(f"{sec} Top 成員", sub, ["排名","code","name","market","heat","hot","near_upper","chg_pct","volume_lots","bid_lots0","ask_lots0","pullback_pct"], height=420)
    ...
    pre = snap[(snap["volume_lots"] >= int(min_lots)) & (snap["dist_upper_pct"].fillna(999) <= float(candidate_dist_pct))].copy()

    if len(pre) < 15:
        pre = snap[(snap["volume_lots"] >= int(min_lots)) & (snap["dist_upper_pct"].fillna(999) <= max(2.0, float(candidate_dist_pct)))].copy()
    ...
    for _, r in pre.iterrows():
        ...
        rows.append({
            ...
            "累積量(張)": int(r["volume_lots"]), "盤中爆量倍數(線性)": float(vol_ratio), "收在高檔(0-1)": float(close_pos),
            "回落(%)": float(pullback_pct), "買一掛單(張)": float(bid_lots0), "賣一掛單(張)": float(ask_lots0),
            "基底天數": int(base_len), "基底緊縮分": float(base_tight), "ATR20(%)": float(atr20),
            "近5日漲幅(%)": float(ret_5d), "連板潛力分": float(score),
        })
...
""", unsafe_allow_html=True)
    ...
    with st.expander("📋 完整榜單（美化表格）", expanded=True):
        cols_show = ["排名","代號","名稱","市場","族群","現價","漲停價","距離漲停(%)","較昨收(%)","累積量(張)","盤中爆量倍數(線性)","收在高檔(0-1)","回落(%)","買一掛單(張)","賣一掛單(張)","基底天數","近5日漲幅(%)","ATR20(%)","基底緊縮分","連板潛力分"]
