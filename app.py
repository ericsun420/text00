# app.py — 起漲戰情室｜戰神修正版 3.8｜最終實戰透明化版 (顯示淘汰名單)
import io
import math
import time
...
# UI / THEME
# =========================
st.set_page_config(page_title="起漲戰情室｜戰神 3.8", page_icon="⚡", layout="wide", initial_sidebar_state="collapsed")
...
.title{ font-size: 42px; font-weight: 900; background: linear-gradient(90deg, #f3f4f6, #9ca3af); -webkit-background-clip:text; -webkit-text-fill-color: transparent; text-align: center; margin:0;}
...
.fail-tag { color: #fb7185; font-size: 13px; font-weight: bold; }
</style>
"""
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                if len(code) == 4 and code.isdigit():
                    ...
                    if ind in ("", "-", "—") or ind.lower() in ("nan", "none"): ind = "未分類"
...
def fast_mis_scan(meta_dict, status_placeholder, now_ts, is_test):
    ...
    for i in range(0, len(codes), batch_size):
        ...
        status_placeholder.update(label=f"📡 盤面掃描中 ({i//batch_size + 1}/{math.ceil(len(codes)/batch_size)})...", state="running")
        try:
...
def core_filter_engine(candidates_df, meta_dict, now_ts, status_placeholder, mis_err, is_test):
    ...
    v_ratio_lim = 0.5 if is_test else 1.3
    pb_lim = 0.05 if is_test else 0.0039
    cp_lim = 0.50 if is_test else 0.80
    ...
    for _, r in candidates_df.iterrows():
        ...
        try:
            ...
            results.append({"代號": c, "名稱": name, "現價": r["last"], "距離(%)": r["dist"], "累積量": int(r["vol_sh"]/1000), "爆量x": vol_ratio, "狀態": "鎖死" if is_locked else "未鎖", "階段": f"第{past_boards+1}連"})
        except: stats["其他錯誤"] += 1
...
is_test = st.checkbox("🔥 **測試模式 (勾選後保證噴標的，讓你確認功能正常)**", value=False)
...
if run_scan:
    ...
    # 【透明化顯示】
    st.markdown("### 📊 掃描統計戰報 (含戰損名單)")
    c1, c2, c3 = st.columns(3)
    c1.metric("初始候選", stats["Total"])
    c2.metric("最終錄取", len(final_res))
    c3.metric("MIS 錯誤", mis_err)
    
    with st.expander("🔍 點開查看：為什麼有些股票不見了？", expanded=True):
        for reason, stocks in stats.items():
            if isinstance(stocks, list) and stocks:
                st.markdown(f"**❌ {reason}**：{', '.join(stocks)}")
            elif isinstance(stocks, int) and stocks > 0 and reason not in ("Total"):
                st.markdown(f"**⚠️ {reason}**：{stocks} 檔")

    if final_res.empty:
        st.warning("⚠️ 目前條件下沒有標的存活。請開啟『測試模式』確認 UI 與資料連線。")
    else:
