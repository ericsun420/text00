...
# =========================
# STOCK LIST (防彈版：解決 OpenAPI 被擋的問題)
# =========================
            ...
            if r.status_code == 200:
                ...
                for line in lines:
                    ...
                    if len(parts) >= 7:
                        ...
                        n = parts[2].replace('"', '').strip()
                        g = parts[6].replace('"', '').strip()
                        if re.match(r"^\d{4,6}$", c):
                            meta.append({"code": c, "name": n, "industry": g})
        except:
            continue
            
    if not meta:
        raise ValueError("無法取得股票清單。請確認雲端伺服器網路連線。")
        
    df = pd.DataFrame(meta).drop_duplicates("code").sort_values("code").reset_index(drop=True)
    return df
...
def build_daily_baseline(codes: list[str]) -> pd.DataFrame:
    ...
    for i in range(0, len(codes), batch):
        ...
        try:
            # 【關鍵修復】：threads=False，徹底解決雲端無限轉圈圈死鎖
            raw = yf.download(
                tickers=tickers, start=start, interval="1d", group_by="ticker",
...
def fetch_intraday_bars_5m(codes: list[str], batch_size: int = 60) -> dict:
    ...
    for i in range(0, len(codes), batch_size):
        ...
        try:
            # 【關鍵修復】：threads=False
            raw = yf.download(
                tickers=tickers, period="1d", interval="5m", group_by="ticker",
                auto_adjust=False, threads=False, progress=False
            )
...
def build_intraday_volume_profile(code: str, lookback_days: int = 20) -> list:
    ...
    try:
        # 【關鍵修復】：threads=False
        raw = yf.download(
            tickers=ticker, period="60d", interval="5m", group_by="ticker",
            auto_adjust=False, threads=False, progress=False
        )
    except Exception:
    ...
    prof = []
    for i in range(54): prof.append(float(sum(s[i] for s in sessions) / len(sessions)))
...
PRESETS = {
    "保守（只抓幾乎鎖死、連板體質強）": dict(
        near_limit_ticks=1, min_close_pos=0.93, max_pullback=0.0025, min_vol_ratio_profile=2.6,
        min_cum_lots=1500, require_break_high60=True, max_ret_5d=8.0, max_atr20=4.8,
        min_base_len=28, min_base_tight=0.55, require_lastN_near_limit=3, max_open_board=1,
    ),
    "標準（平衡：第一根漲停 + 連板機率）": dict(
        near_limit_ticks=1, min_close_pos=0.90, max_pullback=0.0038, min_vol_ratio_profile=2.1,
        min_cum_lots=1200, require_break_high60=False, max_ret_5d=12.0, max_atr20=6.5,
        min_base_len=18, min_base_tight=0.45, require_lastN_near_limit=2, max_open_board=2,
    ),
    "積極（多抓：允許盤中較不穩）": dict(
        near_limit_ticks=2, min_close_pos=0.86, max_pullback=0.0060, min_vol_ratio_profile=1.6,
        min_cum_lots=800, require_break_high60=False, max_ret_5d=18.0, max_atr20=8.5,
        min_base_len=10, min_base_tight=0.35, require_lastN_near_limit=1, max_open_board=4,
    ),
}
...
def scan_first_limitup_continuation(bars_today: dict, base_df: pd.DataFrame, stock_meta: pd.DataFrame, preset: dict, now_ts: datetime) -> pd.DataFrame:
    ...
    for code, df5m in bars_today.items():
        ...
        cond = (
            near_limit and (close_pos >= float(preset["min_close_pos"]))
            and (pullback <= float(preset["max_pullback"])) and (vol_lots >= int(preset["min_cum_lots"]))
            and (vol_ratio_profile >= float(preset["min_vol_ratio_profile"])) and (not yday_was_limit_like)
            and (not had_hype_10d) and (not yday_bad) and (ret_5d <= float(preset["max_ret_5d"]))
            and (atr20 <= float(preset["max_atr20"])) and (base_len >= int(preset["min_base_len"]))
            and (base_tight >= float(preset["min_base_tight"])) and break_high60
            and (lastN_hit >= int(preset["require_lastN_near_limit"])) and (open_board <= int(preset["max_open_board"]))
        )
        ...
        candidates.append({
            "代號": code, "名稱": m.get("name", ""), "族群": m.get("industry", ""), "現價": last,
            "漲停價": limit_up, "距離漲停(%)": dist_pct, "較昨收(%)": chg_pct, "累積量(張)": vol_lots,
            "盤中爆量倍數": float(vol_ratio_profile), "收在高檔(0-1)": float(close_pos), "回落幅度(%)": float(pullback * 100.0),
            "開板次數(5m近似)": int(open_board) if open_board != 999 else None, "連續貼漲停(最後N根)": int(lastN_hit),
            "基底天數": int(base_len), "基底緊縮分": float(base_tight), "ATR20(%)": float(atr20),
            "近5日漲幅(%)": float(ret_5d), "是否突破60日高": bool(break_high60),
        })
    ...
    def score_row(r):
        ...
        s += float(r["族群共振加分"])
        ob = r["開板次數(5m近似)"]
        if ob is not None: s -= min(10.0, float(ob) * 3.0)
...
def load_log() -> pd.DataFrame:
    ...
    try: return pd.read_csv(LOG_PATH, dtype=str)
...
def update_backtest(max_lookahead_days: int = 12) -> pd.DataFrame:
    ...
    for i in range(0, len(codes), 40):
        ...
        try:
            # 【關鍵修復】：threads=False
            raw = yf.download(tickers=tickers, period="6mo", interval="1d", group_by="ticker", auto_adjust=False, threads=False, progress=False)
        except: continue

        for c in chunk:
            try:
                df = raw[f"{c}.TW"].dropna().copy() if isinstance(raw.columns, pd.MultiIndex) else raw.dropna().copy()
                ...
                for d in tasks_by_code.get(c, []):
                    ...
                    seg = df[(df["date"] >= sd) & (df["date"] <= sd + timedelta(days=max_lookahead_days + 2))].copy()
                    ...
                    boards_after = calc_board_count_from_daily(pd.Series([prev_close] + seg["Close"].astype(float).tolist()), limit_pct)
                    new_rows.append({
    ...
    out_all = pd.concat([out_old, out_new], ignore_index=True).drop_duplicates(["signal_date", "code"], keep="last") if not out_old.empty else out_new
...
st.markdown(f"""
<div class="grid">
  <div class="card"><div class="k">股票池</div><div class="v">等待載入</div></div>
...
①貼近漲停( tick ) ②回落小 ③收在高檔 ④同時間量能曲線爆量 ⑤排除昨日先派發 ⑥排除近10日已嗨過 ⑦基底長＋緊縮 ⑧族群共振加權＋回測自動驗證
</div>
""", unsafe_allow_html=True)

# 把執行按鈕放在正中間，又大又明顯
st.markdown("<br>", unsafe_allow_html=True)
run_scan = st.button("🚀 啟動掃描 (套用 1~8 濾網)", use_container_width=True)
...
if run_scan:
    ...
    with st.spinner("鎖定「第一根漲停」＋計算連板潛力分（含同時間量能曲線 / 開板次數 / 族群共振）..."):
        result = scan_first_limitup_continuation(
            bars_today=bars_today, base_df=base, stock_meta=meta, preset=preset, now_ts=now_ts
        )
    ...
    if found == 0:
        ...
        st.success(f"🧊 掃到 {found} 檔「第一根漲停」候選（已按連板潛力分排序），並寫入 scan_data/signals_log.csv")
...
<div class="card">
  <div class="metric">
    <div class="left"><div class="label">#{i} <span class="tag">{tag}</span></div><div class="code">{r['代號']}</div><div class="name">{r['名稱']}</div></div>
    <div style="text-align:right"><div class="price">{float(r['現價']):.2f}</div><div class="chg">漲停 {float(r['漲停價']):.2f}｜距離 {float(r['距離漲停(%)']):.2f}%</div></div>
  </div>
  <div class="hr"></div>
  <div class="small-note">較昨收：{float(r['較昨收(%)']):.2f}% ｜ 累積量：{int(float(r['累積量(張)']))} 張 ｜ 爆量：{float(r['盤中爆量倍數']):.2f}x</div>
  <div class="small-note">開板(5m近似)：{r.get('開板次數(5m近似)', None)} ｜ 連板潛力分：{score:.1f}</div>
</div>
""", unsafe_allow_html=True)
...
st.caption("註：未接券商五檔/封單時，『鎖死』用價格型態近似（tick/回落/收高/開板次數/最後N根貼板）。回測請按上方「更新回測結果」。")
