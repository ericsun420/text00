...
# ✅ 冷酷黑灰｜懶人三段｜卡片＋美化表格｜一鍵掃描同時輸出「第一根漲停候選」與「族群共振」
#
# 安裝：
...
# =========================
# Robust CSV fetch/parse (fix your ParserError)
# =========================
...
def robust_read_csv(text: str) -> pd.DataFrame:
  # guard: if we got HTML (redirect / block page) -> stop early with clear message
  head = text[:800].lower()
  if "<html" in head or "<!doctype html" in head:
    snippet = text[:200].replace("\n", " ")
    raise ValueError(f"抓到的不是 CSV（看起來是網頁/錯誤頁），內容開頭：{snippet}")
  ...
  # parse with python engine (most tolerant)
  try:
    df = pd.read_csv(
      io.StringIO(text),
      sep=delim,
...
def fetch_listed_stocks_mops() -> pd.DataFrame:
  ...
  text = decode_bytes_guess(b)

  df = robust_read_csv(text)

  # validate required columns
  if "公司代號" not in df.columns:
    # sometimes first row becomes one column if content is wrong
    snippet = text[:200].replace("\n", " ")
    raise ValueError(f"CSV 欄位不含『公司代號』，可能內容異常。內容開頭：{snippet}")

  name_col = "公司簡稱" if "公司簡稱" in df.columns else ("公司名稱" if "公司名稱" in df.columns else None)
  if name_col is None:
    raise ValueError(f"CSV 欄位找不到公司名稱欄（公司簡稱/公司名稱）。目前欄位：{df.columns.tolist()[:30]}")
...
# =========================
# FIRST LIMIT-UP SCAN + SECTOR RADAR
# =========================
PRESETS = {
...
def scan_first_limitup_continuation(bars_today: dict, base_idx: pd.DataFrame, meta_df: pd.DataFrame, preset: dict, now_ts: datetime) -> pd.DataFrame:
...
def scan_sector_resonance_radar(bars_today: dict, base_idx: pd.DataFrame, meta_df: pd.DataFrame, now_ts: datetime,
                                heat_threshold: float = 62.0, near_ticks: int = 2, top_sectors: int = 10, top_members: int = 12):
...
if run_scan:
  ...
  with st.spinner("掃描第一根漲停候選（含同時間量能曲線/開板近似/基底排雷）..."):
    result_limit = scan_first_limitup_continuation(bars_today, base_idx, meta, preset, now_ts)
...
""", unsafe_allow_html=True)
  ...
  with st.spinner("掃描族群共振（整個股票池：誰在一起熱、一起貼板）..."):
    sector_rank, sector_members = scan_sector_resonance_radar(
      bars_today, base_idx, meta, now_ts,
      heat_threshold=heat_thr, near_ticks=2, top_sectors=10, top_members=12
    )
    ...
    for sec in sector_rank["族群名稱"].tolist():
      ...
      with st.expander(f"📌 {sec}（Top {len(sub)}）", expanded=False):
        render_table_html(
          f"{sec} Top 成員",
          sub,
          ["排名","代號","名稱","熱度分","熱","貼板","距離漲停(%)","較昨收(%)","盤中爆量倍數(快)","累積量(張)"],
          height=420
        )

st.caption("若再看到『抓上市清單失敗』，新版會直接告訴你：拿到的是 HTML 不是 CSV，並顯示前 200 字方便定位原因。")
